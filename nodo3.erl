  ...
  case PidMain of
    ...
    _ ->
        ...
        case NumeroAmici of
          ...
          2 -> Node = take_one_random(ListaAmici), %io:format("Nodo3: Ho preso a caso: ~p~n", [Node]),
        ...
        receive
          ...
          {dead, DeadFriend} -> %io:format("Nodo3: friend died: ~p~n", [DeadFriend]),
          ...
          %io:format("Nodo3: transazione ricevuta, IDtransazione: ~p~n", [IDtransazione]),
            case lists:member(IDtransazione, TransList) of
              true -> %io:format("Nodo3: Transazione con id: ~p già presente~n", [IDtransazione]),
                      trans_handler(PidMain, ListaAmici, TransList, PidTracker);
              false -> %io:format("Nodo3: Transazione con id: ~p è nuova ~n", [IDtransazione]),
              %io:format("Nodo3: Lista Trans: ~p ~n", [TransList]),
...
block_handler(CatenaNostra, PidChainHandler, Mittente, Blocco) ->
  ...
  %io:format("Nodo3: asking for head to ~p~n", [Mittente]),
    ...
    {trans_list_non_empty, TransList} ->
      ...
      receive
        {catena_empty} -> % first block to mine
          ...
          receive
            {stop_mining} -> %io:format("Nodo3: mining ABORTED!~n"),
              exit(PidMiner, kill);
            {mining_finished, Sol} -> %io:format("Nodo3: Blocco minato!~n"),
...
miner(TransList, IDBlocco, Pid) ->
    ...
    io:format("Nodo3: Mining Finito ~p~n", [Sol]),
